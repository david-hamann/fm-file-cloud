...
- **FR-054**: System MUST allow tenant admins to configure a per-user storage quota; when a user's consumed storage reaches or would exceed that quota, the upload MUST be rejected at the application layer before any storage write is attempted, and the user MUST receive a clear quota-exceeded message; a quota increase MUST take immediate effect, re-enabling uploads within the new limit without requiring any additional action by the user; if a quota is decreased below the user's current usage, existing files MUST remain accessible and the user MUST still be able to delete files, but all new uploads MUST be blocked until consumption falls below the new quota
- **FR-055**: System MUST allow tenant admins to configure a per-group storage quota; the quota applies to the aggregate storage consumed by files directly owned by that group, excluding files owned by descendant subgroups (each subgroup may have its own independent quota); when aggregate storage consumed within a group's context reaches or would exceed that quota, the upload MUST be rejected at the application layer before any storage write is attempted, and the uploading user MUST receive a clear quota-exceeded message; a quota increase MUST take immediate effect, re-enabling uploads within the new limit without requiring any additional action; if a quota is decreased below the group's current usage, existing files MUST remain accessible and deletions MUST still be permitted, but new uploads MUST be blocked until consumption falls below the new quota
- **FR-056**: System MUST allow tenant admins to configure a per-group maximum file size; any individual upload whose size exceeds the configured limit MUST be rejected at the application layer before any storage write is attempted, and the uploading user MUST receive a clear file-size-limit-exceeded message; each such rejection MUST be recorded in the audit log with the actor identity, group scope, attempted file size, configured limit, and timestamp; if the limit is decreased below the size of already-stored files, those files MUST remain accessible and no corrective action is required, but new uploads MUST comply with the updated limit
...
- **ExternalFileAccessKey**: One-time access credential for external read-only sharing. Includes file reference, recipient email, issued key/token, creation timestamp, expiration timestamp (derived from system-configured validity period), used timestamp, and status (active/used/expired/revoked).
